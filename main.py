...
JWT_SECRET = os.environ.get('JWT_SECRET', 'd7e7b147-24c5-472c-afdd-6890dba6048e')
...
def require_jwt(function):
    ...
    def decorated_function(*args, **kws):
        if 'Authorization' not in request.headers:
            abort(401, [{'description': 'Authorization header is expected.'}, 401])
        data = request.headers['Authorization']
        token = str.replace(str(data), 'Bearer ', '')
        try:
            jwt.decode(token, JWT_SECRET, algorithms=['HS256'])
        except:  # pylint: disable=bare-except
            abort(401, [{'description': 'Invalid token.'}, 401])
...
def bad_request_error(error):
    return jsonify({
        "success": False,
        "error": 400,
        "message": error.description[0]['description']
    }), 400
